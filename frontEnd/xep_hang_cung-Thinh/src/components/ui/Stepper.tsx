...
export function Stepper() {
  ...
  const getTextClass = (stepNum: number) => {
    return cn(
      ...
      currentIndex === stepNum ? "block truncate max-w-[120px]" : "hidden md:block",
  ...
  return (
    <div className="bg-card border border-border shadow-sm rounded-xl flex flex-col items-center justify-center px-3 md:px-6 py-1.5 relative w-full max-w-[1248px] mx-auto">
      <div className="w-full max-w-[944px] flex items-center justify-center gap-[4px] md:gap-[8px] relative overflow-hidden">
        ...
        <button type="button" onClick={() => handleStepClick("step1")} className={getButtonClass()}>
          <div className={cn(
            "relative shrink-0 size-6 flex items-center justify-center rounded-full transition-colors",
            isStep1 ? "bg-primary/10" : "bg-muted"
          )}>
            <Truck className={cn("size-3.5 transition-colors", isStep1 ? "text-primary" : "text-muted-foreground")} strokeWidth={2.5} />
          </div>
          <div className={getTextClass(1)}>
            Chọn xe
          </div>
        </button>
        ...
        {/* Step 2: Nhập Hàng */}
        <button type="button" onClick={() => handleStepClick("step2")} className={getButtonClass()}>
          <div className={cn(
            "relative shrink-0 size-6 flex items-center justify-center rounded-full transition-colors",
            isStep2 ? "bg-primary/10" : "bg-muted"
          )}>
            <PackageOpen className={cn("size-3.5 transition-colors", isStep2 ? "text-primary" : "text-muted-foreground")} strokeWidth={2.5} />
          </div>
          <div className={getTextClass(2)}>
            Nhập hàng
          </div>
        </button>
        ...
        {/* Step 3: Kết Quả */}
        <button type="button" onClick={() => handleStepClick("step3")} className={getButtonClass()}>
          <div className={cn(
            "relative shrink-0 size-6 flex items-center justify-center rounded-full transition-colors",
            isStep3 ? "bg-primary/10" : "bg-muted"
          )}>
            <ClipboardCheck className={cn("size-3.5 transition-colors", isStep3 ? "text-primary" : "text-muted-foreground")} strokeWidth={2.5} />
          </div>
          <div className={getTextClass(3)}>
            Kết quả
          </div>
        </button>
