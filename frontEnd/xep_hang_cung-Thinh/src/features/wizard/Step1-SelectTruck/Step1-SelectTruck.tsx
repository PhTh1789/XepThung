...
export function Step1SelectTruck() {
  ...
  const activateCustomTruckMode = useCargoStore(state => state.activateCustomTruckMode);
  ...
  return (
    <div className="flex flex-col w-full max-w-[1280px] mx-auto">
      {/* Section Header */}
      <div className="sticky top-0 z-30 flex flex-col items-start w-full bg-background/85 backdrop-blur-xl pb-4 pt-3 sm:pt-4 -mt-3 sm:-mt-4 px-3 sm:px-4 -mx-3 sm:-mx-4 border-b border-border rounded-t-2xl">
      ...
      {/* Bento Grid Layout cho việc chọn xe */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 auto-rows-fr gap-4 w-full pt-4">
        {/* Card: Form Tùy chỉnh (Luôn nằm ở vị trí đầu tiên của Grid) */}
        <TruckForm 
          isSelected={truckMode === "custom"} 
          onActivate={activateCustomTruckMode} 
        />

        {/* Danh sach cac Xe Tu Thu Vien (Gom ca Saved + Presets) */}
